...
class Application(Frame):
    ...
    def createWidgets(self):
        ...
        self.show_full_content = BooleanVar(value=True)
        self.show_full_content_checkbutton = Checkbutton(self, text="show full content", variable=self.show_full_content)
        self.show_full_content_checkbutton.pack(side=TOP, anchor=W, expand=0)

        self.mark_replacements = BooleanVar(value=True)
        self.mark_replacements_checkbutton = Checkbutton(self, text="mark replacements", variable=self.mark_replacements)
        ...
        self.text = Text(self.text_frame, yscrollcommand=self.scrollbar.set)

        self.text.tag_config("file", foreground="grey")
        self.text.tag_config("search", background="yellow", foreground="black")
        self.text.tag_config("replace", foreground="blue")
    ...
    def do_preview(self):
        ...
        for path, dirs, files in os.walk(directory):
            for fname in files:
                ...
                if not self.glob.get() or fnmatch.fnmatch(fname.lower(), self.glob.get()):
                    print("%s" % filename)

                    with open(filename, 'rt', encoding='latin-1') as fin:
                        lines = fin.read().splitlines()
                    lines = minised.minised_on_lines(lines, self.search.get(), self.replace.get(), 
