...
#define HEXHTML_ASSIGN \
        ...
        if(fdnum < 0) { \
            hexhtml      = &memory_file[-fdnum].hexhtml; \
            hexhtml_size = &memory_file[-fdnum].hexhtml_size; \
        } else { \
            hexhtml      = &filenumber[fdnum].hexhtml; \
            hexhtml_size = &filenumber[fdnum].hexhtml_size; \
        } \
...
int hexhtml_build(int fdnum) {
    ...
    if(fdnum < 0) {
        ...
        name = filenumber[fdnum].filename;
        if(!name) {
            name = filenumber[fdnum].basename;
            if(!name) {
                name = filenumber[fdnum].fullname;
                if(!name) name = "FILE";
            }
        }
    }
    ...
    if(hexhtml_output == HEXHTML_CONSOLE) {
        ...
        for(i = 0; i <= (hexhtml_idx_max + 1); i++) {
            for(j = 0;; j++) {
                ...
                sprintf(tmp, p, enable_hexhtml ? "a" : "td", i);
                if(!i) {
                    tmp[0] = ' ';
                    tmp[1] = ' ';
                    tmp[2] = ' ';
                    sprintf(tmp + 3, "%s", enable_hexhtml ? "a" : "td");
                }
    ...
    if(enable_hexhtml >= 0) goto hexhtml_other_types;
    ...
    if(hexhtml_output == HEXHTML_CONSOLE) {
        ...
        CONSOLE_SCREEN_BUFFER_INFO  hexhtml_console_info = {{0}};
        HANDLE          hexhtml_stdout  = (HANDLE)NULL,
                        hexhtml_stdin   = (HANDLE)NULL;
...
hexhtml_quit:
    ...
    if(fd) {
        if((fd != stdout) && (fd != stderr)) fclose(fd);
    }
    fprintf(stderr, "- done\n");
    return(0);
}
