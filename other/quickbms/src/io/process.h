...
SIZE_T readwrite_mem(HANDLE hp, u8 *addr, void *buff, SIZE_T size, int w) {
    ...
    for(i = 0; i < size; i += len) {
        len = g_page_size - ((SIZE_T)addr & (g_page_size - 1));
        ...
        if(!rw) {
            attr = PAGE_READWRITE;
            if(VirtualQueryEx(hp, addr + i, &mbi, len)) {
                attr = mbi.Protect;
                switch(attr & 0xff) {
            ...
            if(VirtualProtectEx(hp, addr + i, g_page_size, attr, &old_attr)) {

                if(w) WriteProcessMemory(hp, addr + i, buff + i, len, &t);
                else  ReadProcessMemory (hp, addr + i, buff + i, len, &t);

                VirtualProtectEx(hp, addr + i, g_page_size, old_attr, &attr);
                if(w) {
...
quick_thread(process_debugger, process_file_t *proc) {
    ...
    while(proc->debug) {
        ...
        // Process exit
            ...
            if(verbose) printf("DEBUG debug string  %s\n",
                p);
